...
Game animation keys are then bound to tag paths in the "-- Key Bindings --" list
(cycle with < / >). At runtime `AssetDefinition::resolved_clip` maps a game key ->
bound tag path -> the clip carrying that tag. Persisted as `clip_tags` +
