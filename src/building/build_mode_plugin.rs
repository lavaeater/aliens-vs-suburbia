use crate::building::systems::{
    build_tower_system, building_mode, change_build_indicator, enter_build_mode, execute_build,
    exit_build_mode,
};
use crate::game_state::GameState;
use crate::general::systems::map_systems::{add_tile_to_map, remove_tile_from_map};
use crate::player::events::building_events::{
    AddTile, ChangeBuildIndicator, EnterBuildMode, ExecuteBuild, ExitBuildMode, RemoveTile,
};
...
impl Plugin for BuildModePlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(BuildModeEventsPlugin).add_systems(
            Update,
            (
                enter_build_mode,
                exit_build_mode,
